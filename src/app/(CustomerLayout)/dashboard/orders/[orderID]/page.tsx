...
const STATUS_MAP: Record<number, { label: string; variant: "available" | "new" | "outOfStock" | "default" | "secondary"; icon: React.ElementType; color: string }> = {
	1: { label: "در انتظار پرداخت", variant: "outOfStock", icon: Clock, color: "text-amber-600" },
	2: { label: "پرداخت شده", variant: "new", icon: CreditCard, color: "text-blue-600" },
	3: { label: "ارسال شده", variant: "new", icon: Truck, color: "text-blue-600" },
	4: { label: "تحویل داده شده", variant: "available", icon: CheckCircle2, color: "text-green-600" },
	5: { label: "لغو شده", variant: "secondary", icon: XCircle, color: "text-muted-foreground" },
}
								...
								{STATUS_STEPS.map((step) => {
									...
									return (
										<div key={step.status} className="flex flex-col items-center gap-2 relative z-10">
											...
											<span className={`text-xs text-center max-w-[60px] ${done ? "font-semibold text-primary-rose" : "text-muted-foreground"}`}>
												{step.label}
											</span>
										</div>
									)
				...
				<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
					<Card className="border-amber-500/40 bg-amber-500/5">
						<CardContent className="p-4 flex items-center justify-between gap-4">
							<div>
								<p className="font-semibold text-amber-700 dark:text-amber-400">این سفارش هنوز پرداخت نشده</p>
								<p className="text-sm text-muted-foreground">برای تکمیل سفارش، یکی از روش‌های پرداخت زیر را انتخاب کنید</p>
							</div>
							<div className="flex gap-2 flex-shrink-0">
								<Button variant="outline" size="sm" className="gap-1" onClick={handlePayWallet} disabled={paying}>
									<Wallet className="w-4 h-4" />
									کیف پول
								</Button>
								<Button variant="luxury" size="sm" className="gap-1" onClick={handlePayGateway} disabled={paying}>
			...
			<div className="grid md:grid-cols-3 gap-6">
				...
				>
					<Card>
						<CardContent className="p-4 space-y-3">
							...
							<div className="flex justify-between text-sm">
								...
								<span>{PAYMENT_METHOD_MAP[order.paymentMethod] ?? "—"}</span>
							</div>
							<div className="flex justify-between text-sm">
								<span className="text-muted-foreground">هزینه ارسال</span>
								<span>{order.shippingCost > 0 ? `${formatPrice(order.shippingCost)} تومان` : "رایگان"}</span>
							</div>
							{order.refundFlag && (
							...
							<div className="flex justify-between font-bold">
								<span>جمع کل</span>
								<span className="gradient-text">{formatPrice(order.totalAmount)} تومان</span>
							</div>
						</CardContent>
					</Card>
					...
					{order.statusHistory && order.statusHistory.length > 0 && (
						<Card>
							<CardContent className="p-4 space-y-3">
								<h2 className="font-bold text-sm">تاریخچه وضعیت</h2>
								<div className="space-y-2">
									{order.statusHistory.map((h, i) => {
										const s = STATUS_MAP[h.status]
										return (
											<div key={i} className="flex items-start gap-2">
												<div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${s?.color ?? "bg-muted"}`} />
												<div>
