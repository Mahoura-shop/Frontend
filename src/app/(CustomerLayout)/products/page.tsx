...
import {
  Heart, ShoppingBag, Star, Filter, Search, Grid3x3, List,
  SlidersHorizontal, X, Eye, Package
} from 'lucide-react';
...
export default function ProductsPage() {
  ...
  const categories = ['all', 'آرایش صورت', 'مراقبت از پوست', 'آرایش چشم', 'عطر و ادکلن'];
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (priceRange !== 'all') {
      filtered = filtered.filter((p) => {
        ...
        if (priceRange === 'mid') return p.price >= 300000 && p.price < 400000;
                  ...
                  <div className="mb-6">
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        { value: 'low', label: 'زیر ۳۰۰,۰۰۰ تومان' },
                        { value: 'mid', label: '۳۰۰,۰۰۰ - ۴۰۰,۰۰۰ تومان' },
                        { value: 'high', label: 'بالای ۴۰۰,۰۰۰ تومان' },
                      ].map((range) => (
                        <button
                          key={range.value}
                      ...
                      <Card className="overflow-hidden group hover:shadow-xl transition-all duration-300">
                        <div className="relative">
                          <Link href={`/products/${product.id}`}>
                            <div className={`relative ${viewMode === 'grid' ? 'h-80' : 'h-60 md:h-80'} overflow-hidden cursor-pointer`}>
                              <motion.img
                                src={product.image}
                                alt={product.name}
