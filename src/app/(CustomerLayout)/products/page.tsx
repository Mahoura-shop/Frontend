...
function mapSort(sort: string): string {
	switch (sort) {
		...
		case "most-visited": return "most_visited";
...
export default function ProductsPage() {
	...
	const [selectedBrand, setSelectedBrand] = useState(() => searchParams.get("brand") ?? "0");
	...
	const refreshProducts = useCallback(async () => {
		setIsLoading(true);
		try {
			await fetchProducts({ q: debouncedSearch, categoryID: selectedCategory !== "0" ? Number(selectedCategory) : undefined, brandID: selectedBrand !== "0" ? Number(selectedBrand) : undefined, sortBy: mapSort(sortBy), limit: ITEMS_PER_PAGE, offset: (currentPage - 1) * ITEMS_PER_PAGE });
	...
	const handleAddToCart = async (e: React.MouseEvent, productId: number) => {
		...
		if (!product) return;
		if (getCartCount(productId) >= product.quantity) { CustomToast("موجودی کافی نیست", "error"); return; }
		setAddingId(productId);
		try { await addItem(productId); }
	...
	const handleToggleWishlist = async (e: React.MouseEvent, productId: number) => {
		...
		} catch {} finally { setWishlistingId(null); }
	};

	const handleClearFilters = () => {
		setSearchQuery(""); setSelectedCategory("0"); setSelectedBrand("0");
	...
	useEffect(() => { fetchCategories(); fetchBrands(); if (accessToken) fetchCart(); }, []);

	useEffect(() => {
		if (!products || products.length === 0) { setSliderBounds({ min: 0, max: 5000000 }); return; }
		const max = Math.max(...products.map((p) => resolvePrice(p, userType)));
		setSliderBounds({ min: 0, max });
	}, [products, userType]);

	useEffect(() => { setPriceRange([0, sliderBounds.max]); }, [sliderBounds.max]);

	useEffect(() => {
		if (!accessToken) return;
		getWishlist().then((res) => setWishlistIds(new Set((res?.data ?? []).map((item: any) => item.product.id)))).catch(() => {});
	}, [accessToken]);
	...
	useEffect(() => {
		const load = async () => {
			setIsLoading(true);
			try {
				await fetchProducts({ q: debouncedSearch, categoryID: selectedCategory !== "0" ? Number(selectedCategory) : undefined, brandID: selectedBrand !== "0" ? Number(selectedBrand) : undefined, minPrice: priceRange[0] > 0 ? priceRange[0] : undefined, maxPrice: priceRange[1] < 100000 ? priceRange[1] : undefined, sortBy: mapSort(sortBy), limit: ITEMS_PER_PAGE, offset: (currentPage - 1) * ITEMS_PER_PAGE });
			} finally { setIsLoading(false); }
		};
		load();
	}, [debouncedSearch, selectedCategory, selectedBrand, priceRange, sortBy, currentPage]);

	const filterProps = { selectedCategory, selectedBrand, priceRange, sliderBounds, categories, brands, onCategoryChange: setSelectedCategory, onBrandChange: setSelectedBrand, onPriceChange: (v: number[]) => { setPriceRange(v); setCurrentPage(1); }, onClear: handleClearFilters };
	const totalPages = Math.ceil(totalCount / ITEMS_PER_PAGE);

	return (
		<div className="min-h-screen bg-background">
			{(pulling || refreshing) && (
				<div className="fixed top-0 inset-x-0 z-50 flex items-center justify-center pointer-events-none" style={{ height: pullY || (refreshing ? 56 : 0), transition: pulling ? "none" : "height 0.3s ease" }}>
			...
			<div className="bg-gradient-to-r from-primary-rose/20 via-accent-gold/10 to-secondary-plum/20 py-8 md:py-32 md:pb-16">
				<div className="container mx-auto px-4 pt-8">
					<motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center">
						<h1 className="text-3xl md:text-5xl font-bold gradient-text mb-4">تمامی محصولات</h1>
			...
			<div className="container mx-auto px-4 py-6 md:py-12">
				<div className="flex flex-col lg:flex-row gap-8">
					...
					<MobileFilterSheet {...filterProps} open={showFilters} onClose={() => setShowFilters(false)} />

					<div className="flex-1">
						<ProductsToolbar searchQuery={searchQuery} sortBy={sortBy} onSearchChange={setSearchQuery} onSortChange={setSortBy} onShowFilters={() => setShowFilters(true)} />
