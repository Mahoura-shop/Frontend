    ...
    <div className="min-h-screen bg-background">
      <div className="bg-gradient-to-r from-primary-rose/20 via-accent-gold/10 to-secondary-plum/20 py-24">
        <div className="container mx-auto px-4">
          ...
          >
            <h1 className="text-5xl md:text-6xl font-bold gradient-text mb-6">برندها</h1>
            <p className="text-xl text-muted-foreground leading-relaxed">
              مجموعه برندهای معتبر و باکیفیت موجود در فروشگاه ماهورا
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-16">
        {activebrands.length === 0 ? (
          <p className="text-center text-muted-foreground">برندی یافت نشد</p>
        ) : (
                ...
                <Link href={`/products?brand=${brand.id}`}>
                  <Card className="h-full hover:shadow-xl transition-shadow cursor-pointer group">
                    <CardContent className="p-6 flex flex-col items-center gap-3">
                      ...
                      <h3 className="text-sm font-bold text-center leading-tight">{brand.name}</h3>
                      {brand.description && (
