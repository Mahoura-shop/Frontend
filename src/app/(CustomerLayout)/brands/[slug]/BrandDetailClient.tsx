...
export default function BrandDetailClient() {
	...
	const loadProducts = useCallback(async (brandId: number, page: number) => {
		setIsLoading(true);
		try {
			await fetchProducts({ brandID: brandId, limit: ITEMS_PER_PAGE, offset: (page - 1) * ITEMS_PER_PAGE });
		} finally {
			setIsLoading(false);
		}
	}, [fetchProducts]);
	...
	useEffect(() => {
		if (!brand) return;
		loadProducts(brand.id, currentPage);
			...
			<div className="bg-gradient-to-r from-primary-rose/20 via-accent-gold/10 to-secondary-plum/20 py-24">
				<div className="container mx-auto px-4">
					...
					>
						...
						<div>
							<h1 className="text-4xl text-center leading-loose md:text-5xl font-bold gradient-text pb-3">{brand.name}</h1>
							{brand.description && (
								<p className="text-lg text-muted-foreground leading-relaxed text-justify">{brand.description}</p>
							)}
						</div>
			...
			<div className="container mx-auto px-4 py-12">
				{isLoading ? (
					...
					<div className="text-center py-20">
						...
						<p className="text-muted-foreground">این برند در حال حاضر محصولی ندارد</p>
					</div>
				) : (
