...
export default function SettingsPage() {
  ...
  const handleRateChange = (code: string, value: string) => {
    ...
    setEditedRates((prev) =>
      prev.map((rate) => (rate.code === code ? { ...rate, rate: numValue } : rate))
    );
  };
      ...
      >
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              نرخ تبدیل ارزهای مختلف به ریال ایران. قیمت محصولات بر اساس این نرخ‌ها تبدیل می‌شود.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid md:grid-cols-2 gap-6">
              {editedRates.map((currency, i) => (
                <motion.div
                  ...
                  className="space-y-2"
                >
                  <Label htmlFor={currency.code} className="flex items-center justify-between">
                    <span className="flex items-center gap-2">
                      <span className="text-2xl">{currency.symbol}</span>
                      <span className="font-bold">{currency.nameFa}</span>
                      <span className="text-sm text-muted-foreground">({currency.code})</span>
                    </span>
                  </Label>
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-muted-foreground whitespace-nowrap">1 {currency.code} =</span>
                    <Input
                      id={currency.code}
                      type="number"
                      value={currency.rate}
                      ...
                      className="text-left"
                    />
                    <span className="text-sm text-muted-foreground whitespace-nowrap">ریال</span>
                  </div>
                  {currency.code !== 'IRR' && (
                    <p className="text-xs text-muted-foreground">
                      ۱۰۰ {currency.code} = {new Intl.NumberFormat('fa-IR').format(currency.rate * 100)} ریال
                    </p>
                  )}
                </motion.div>
              ))}
      ...
      >
        <Card>
          <CardHeader>
            ...
            <CardDescription>نمونه‌ای از تبدیل قیمت‌ها</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid md:grid-cols-3 gap-4">
              {editedRates
                ...
                .map((currency) => {
                  ...
                  const irrPrice = examplePrice * currency.rate;
                  return (
                    <div
                      key={currency.code}
                    ...
                    >
                      ...
                      <p className="text-sm">
                        ...
                        <span className="font-bold gradient-text">
                          {new Intl.NumberFormat('fa-IR').format(irrPrice)}
                        </span>{' '}
                        ریال
                      </p>
                    </div>
